let itemsToExchange = ['mistletoe', 'candycane', 'candy0', 'candy1', 'gem0', 'gem1', 'leather', 'seashell', 'basketofeggs', 'armorbox', 'weaponbox'];

let lastExchange = 0;
setInterval(function () {
	if (character.ctype !== 'merchant') return;
	if (character.map === 'bank') return;
	if (character.esize <= 4) return;
	if (!character.q.exchange && new Date() - lastExchange > 1000) {
		for (let id in itemsToExchange) {
			...
			let exchangeable = matches.find(function (e) {
				return e.q >= itemDef.e
			});
			if (exchangeable) {
