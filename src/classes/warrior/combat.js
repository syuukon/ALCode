...
function farm()
{
	...
	if (!is_on_cooldown('hardshell') && character.hp / character.max_hp < 0.5 && character.mp > G.skills.hardshell.mp + 50) {
		use_skill('hardshell');
	}
    ...
    if (target != null) {
        if (distance_to_point(target.real_x, target.real_y) < character.range) {
            if (can_attack(target)) {
				if (!is_on_cooldown('taunt') && target.target !== 'SyuuTank') {
					use_skill('taunt');
				}
                attack(target);
            }
